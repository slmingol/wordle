...
export function initializeAnalytics(userConfig: Partial<AnalyticsConfig> = {}): void {
	...
	if (config.respectDNT && isDNTEnabled()) {
		...
		if (import.meta.env.DEV) {
			console.warn("[Analytics] Disabled due to Do Not Track");
		}
		return;
	}
	...
	if (!config.enabled || !consentGiven) {
		if (import.meta.env.DEV) {
			console.warn(
				"[Analytics] Not initialized - consent:",
				consentGiven,
				"enabled:",
				config.enabled
			);
	...
	if (import.meta.env.DEV) {
		console.warn("[Analytics] Initialized with provider:", config.provider);
	}
}
...
export function trackPageView(url?: string): void {
	...
	if (import.meta.env.DEV) {
		console.warn("[Analytics] Page view:", pageUrl);
	}
}
...
export function revokeConsent(): void {
	...
	const scripts = document.querySelectorAll(
		'script[src*="plausible"], script[src*="fathom"], script[src*="googletagmanager"]'
	);
	scripts.forEach((script) => script.remove());

	if (import.meta.env.DEV) {
		console.warn("[Analytics] Consent revoked");
	}
}
...
export const GameEvents = {
	...
	GAME_WON: (mode: string, guesses: number) =>
		trackEvent({
			name: "game_won",
			properties: { mode, guesses },
		}),
	GAME_LOST: (mode: string) =>
		trackEvent({
			name: "game_lost",
			properties: { mode },
		}),
	GAME_SHARED: (mode: string) =>
		trackEvent({
			name: "game_shared",
			properties: { mode },
		}),
	SETTINGS_CHANGED: (setting: string, value: string | boolean) =>
		trackEvent({
